...
> 状态：已实施，知识脊柱 Gate 已通过。
>
> 实施基线：`feature/knowledge-spine`，提交范围 `d854947` → `6be9561`。
>
> 后续个人知识库唯一方案见 [personal_knowledge_base_plan.md](personal_knowledge_base_plan.md)。本文档继续作为底座设计与验收记录，不再定义后续 P1 / P2 / P3 的实现细节。
...
知识脊柱只定义后续能力可以依赖的事实和入口。个人知识库的当前演进顺序已经收敛到 [personal_knowledge_base_plan.md](personal_knowledge_base_plan.md)：

1. P1 高价值知识视图；
2. P2 学习生命周期与成长；
3. P3 关联、方向反馈与语义能力。

后续阶段的范围、禁止项和 Gate 以该文档为准。本设计中较早的 score、生命周期和关联设想不再单独作为实施依据。

---

## 12. Gate 验收记录

- [x] 本 Gate 未改变收藏、复习和 UI 可见行为。
- [x] 重复术语回链缺失和同 capture 重试虚增均先建立失败测试。
- [x] 真实数据库副本迁移无损，旧 `captures.context_id` 保持 `NULL`。
- [x] `review_events` 与 SRS 快照更新共用事务。
- [x] `KnowledgeBase` 不依赖 PySide6、AIClient 或 OCR。
- [x] worker 的术语与 tip 写入收敛为一次 ingest，没有保留双写。
- [x] UI / worker 不再直接调用低层知识写方法。
- [x] 实施验收时 186 项测试通过（163 项原有 + 23 项新增）。
- [x] 实施验收时完成真实数据库迁移、知识表 `foreign_key_check` 和桌面 smoke。

阶段提交：

| 阶段 | 提交 | 结果 |
|---|---|---|
| 设计 | `d854947` | 确定最小知识脊柱和 Gate |
| A + B | `ca680f9` | 修复来源/幂等；增加方向与复习事实 |
| C | `1f12c50` | 新建 `KnowledgeBase` 并统一 worker ingest |
| D | `d18f9e0` | UI 调用迁移到知识入口 |
| E | `6be9561` | 低层方法私有化、测试收口、Gate 完成 |

验收时发现的 `conversations` 历史孤儿记录属于删除流程的独立完整性问题，不影响知识表 Gate；后续单独修复，不并入个人知识库 P1。
