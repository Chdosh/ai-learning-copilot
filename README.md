...
- **追问对话**：对每条截图记录连续提问
- **术语积累**：自动提取术语与中文名，支持收藏、出处回溯和 Anki 导出
- **间隔复习**：收藏术语按遗忘曲线自动排期，今日复习卡片（忘了 / 模糊 / 记得）
- **学习建议**：AI 建议自动沉淀为清单，可完成 / 忽略，并可一键合并进学习方向背景要点
- **学习方向**：按领域 / 场景配置回答背景与偏好，截图时自动识别方向并提示冲突
...
- API Key 本地加密存储，非明文，不过建议创建一个新的单独管理。
